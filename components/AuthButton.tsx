...
export default function AuthButton() {
  ...
  const [premiumSynced, setPremiumSynced] = useState(false);
  ...
  useEffect(() => {
    const unsub = onAuthChange(async (u) => {
      setUser(u);
      if (u) {
        const restored = await checkFirestorePremium();
        if (isPremium()) {
          ...
          if (token) {
            ...
            try {
              ...
              // Non-critical — restore path still works
            }
          }
        }
        setPremiumSynced(restored || isPremium());
      }
    });
    return () => {
      unsub?.();
    ...
    return (
      ...
      >
        ...
        <p className="text-xs" style={{ color: "var(--text-muted)" }}>
          Cross-device sync will be available once Firebase is set up.
        </p>
      </div>
    );
  }
